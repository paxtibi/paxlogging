...
type
  ...
  protected
    ...
    procedure CallAppenders(const Event: TLogEvent); virtual;
  ...
  public
    ...
    property TraceLevel: longint read FTraceLevel;
    ...
    procedure Enter(const Message: string); overload;
    procedure Leave(const Message: string); overload;
    procedure Enter(const obj: TObject; const methodName: string); overload;
    procedure Leave(const obj: TObject; const methodName: string); overload;
  end;
...
uses
  StreamIO, paxlog_appender, paxlog_resources, paxlog_dispatcher;
...
procedure TLogOptionHandler.Initialize;
begin
...
var
  {Event: TLogEvent;}
  dispatcher: TPaxLoggerAppenderDispatcher;
begin
  dispatcher := TPaxLoggerAppenderDispatcher.Create();
  dispatcher.Log := self;
  dispatcher.Event := TLogEvent.Create(Self, LogLevel, Message, Err);
  dispatcher.Run;
  {
...
begin
  Result := IsEnabledFor(paxlog.Debug);
end;
...
procedure TLogLogger.Enter(const Message: string);
begin
  Trace('Enter ' + Message);
  FTraceLevel += 1;
end;
...
procedure TLogLogger.Enter(const obj: TObject; const methodName: string);
begin
  if obj <> nil then
    Enter(obj.ClassName + '.' + methodName)
  else
    Enter('nil.' + methodName);
end;

procedure TLogLogger.Leave(const obj: TObject; const methodName: string);
begin
  if obj <> nil then
    Leave(obj.ClassName + '.' + methodName)
  else
...
begin
  if (LogLevel <> paxlog.Debug) or InternalDebugging then
...
procedure TLogCustomLayout.Initialize;
begin
...
procedure TLogPatternLayout.Initialize;
begin
  inherited Initialize;
